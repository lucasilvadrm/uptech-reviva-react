...
jest.mock('contexts/CartContext', () => ({
  useCart: () => { return { cart: propsMock, addProductInCart: jest.fn() } }
}))
...
describe('Testes no componente ProductListItem', () => {
  ...
  it('teste renderização e função de click', () => {
    ...
    // const grupoTamanhos = screen.getAllByRole('group');

    userEvent.click(image);
    expect(mockedUsedNavigate).toHaveBeenCalledTimes(1);
    expect(mockedUsedNavigate).toHaveBeenCalledWith("/details/1", { state: { product: propsMock[0] } });

    // expect(title).toBeInTheDocument();

    // expect(grupoTamanhos.length).toBe(1);
